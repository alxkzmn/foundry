...
pub struct Ext8ArithmeticVector {
    pub packed_a: String,
    pub packed_b: String,
    pub scalar: String,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct Ext8ArithmeticVectorFile {
    pub seed: u64,
    pub count: usize,
    pub vectors: Vec<Ext8ArithmeticVector>,
}

sol! {
    struct Ext8ArithmeticVectorsAbi {
        uint256[] packedA;
        uint256[] packedB;
        uint256[] scalars;
        uint256[] packedAdd;
...
pub fn generate_ext8_arithmetic_vectors(count: usize) -> Ext8ArithmeticVectorFile {
    let mut rng = XorShift64::new(VECTOR_SEED);
    ...
    for _ in 0..count {
        ...
        vectors.push(Ext8ArithmeticVector {
            packed_a: encode_hex_word(&encode_packed_ext8_word(&a)),
            packed_b: encode_hex_word(&encode_packed_ext8_word(&b)),
            scalar: format!("0x{:064x}", scalar.as_canonical_u32()),
            packed_add: encode_hex_word(&encode_packed_ext8_word(&(a + b))),
            packed_sub: encode_hex_word(&encode_packed_ext8_word(&(a - b))),
            packed_mul: encode_hex_word(&encode_packed_ext8_word(&(a * b))),
            packed_square_a: encode_hex_word(&encode_packed_ext8_word(&(a * a))),
            packed_mul_base: encode_hex_word(&encode_packed_ext8_word(&(a * scalar))),
        });
    }
    Ext8ArithmeticVectorFile { seed: VECTOR_SEED, count, vectors }
}

pub fn write_vector_outputs(out_dir: &Path, count: usize) -> anyhow::Result<()> {
    let file = generate_ext8_arithmetic_vectors(count);
    let json_path = out_dir.join("ext8_precompile_vectors.json");
    let abi_path = out_dir.join("ext8_precompile_vectors.abi");

    let encoded = serde_json::to_vec_pretty(&file)?;
    fs::write(&json_path, encoded)
        .with_context(|| format!("failed to write {}", json_path.display()))?;

    let abi = Ext8ArithmeticVectorsAbi {
        packedA: file
            .vectors
            .iter()
